    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setDlgState) => AlertDialog(
        ...
        content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
          const Text('Food Quality', style: TextStyle(fontWeight: FontWeight.w700)),
          _StarRow(rating: foodRating, onChanged: (r) => setDlgState(() => foodRating = r)),
          const SizedBox(height: 12),
          if (order.deliveryType == 'delivery') ...[
            const Text('Delivery', style: TextStyle(fontWeight: FontWeight.w700)),
            _StarRow(rating: deliveryRating, onChanged: (r) => setDlgState(() => deliveryRating = r)),
            const SizedBox(height: 12),
          ],
          const Text('Overall', style: TextStyle(fontWeight: FontWeight.w700)),
          _StarRow(rating: overallRating, onChanged: (r) => setDlgState(() => overallRating = r)),
          const SizedBox(height: 12),
          TextField(
          ...
          ElevatedButton(
            onPressed: () async {
              if (foodRating == 0 || overallRating == 0) {
                showSnack(context, 'Please rate food quality and overall experience', isError: true);
                return;
              }
              Navigator.pop(ctx);
              try {
                await ApiService.submitOrderFeedback(
                  ...
                  deliveryRating: order.deliveryType == 'delivery' && deliveryRating > 0 ? deliveryRating : null,
        ...
        child: ListView(
          ...
          children: [
            ...
            _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Icon(
                  order.isCOD ? Icons.payments_outlined : Icons.credit_card_outlined,
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: order.isPaid
                      ...
                      : const Color(AppColors.warning).withOpacity(0.12),
                ...
                ...List.generate(_statusSteps.length, (i) {
                  ...
                  return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Column(children: [
                      Container(
                        ...
                        child: Center(child: done
                          ...
                          : Text('${i + 1}', style: TextStyle(color: Colors.grey.shade500, fontSize: 12, fontWeight: FontWeight.w700))),
                      ),
                    ...
                    Padding(
                      ...
                      child: Text(_stepLabel(_statusSteps[i]), style: TextStyle(
              ...
              _card(Row(children: [
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  if (order.coinsRedeemed > 0)
                    Text('${order.coinsRedeemed} coins redeemed (₹${order.coinsRedeemed} off)',
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(AppColors.coins))),
                  if (order.coinsEarned > 0)
                    Text('${order.coinsEarned} coins earned on this order',
                      style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Color(AppColors.success))),
                  if (order.coinsEarned == 0 && order.isDelivered)
                    Text('Coins will be credited after delivery', style: TextStyle(fontSize: 12, color: Colors.grey.shade500)),
                ])),
              ])),
            if (order.coinsRedeemed > 0 || order.coinsEarned > 0) const SizedBox(height: 12),
              ...
              ...order.items.map((item) => Padding(
                ...
                child: Row(children: [
                  ...
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    if ((item.toppings as List).isNotEmpty)
                      Text('+ ${(item.toppings as List).map((t) => t['topping_name'] ?? t['name'] ?? '').join(', ')}',
            ...
            _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (order.discountAmount > 0) ...[
                ...
                PriceRow(label: 'Coupon Discount', amount: -order.discountAmount, color: const Color(AppColors.success)),
              ],
              if (order.coinsRedeemed > 0) ...[
                const SizedBox(height: 6),
                PriceRow(label: '🪙 Coins Redeemed', amount: -order.coinsRedeemed.toDouble(), color: const Color(AppColors.coins)),
              ],
              const SizedBox(height: 6),
            ...
            _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              if (order.locationName != null) _infoRow(Icons.store_outlined, 'Branch', order.locationName!),
              _infoRow(Icons.delivery_dining_outlined, 'Type', order.deliveryType == 'delivery' ? 'Home Delivery' : 'Pickup'),
              if (order.deliveryAddress != null) _infoRow(Icons.location_on_outlined, 'Address', order.deliveryAddress!),
              _infoRow(Icons.payment_outlined, 'Payment', order.isCOD ? 'Cash on Delivery' : 'Online'),
            ])),
            ...
            if (order.isDelivered) ...[
              ...
              order.feedback != null
                ? _card(Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    const Text('Your Feedback', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 15)),
                    const SizedBox(height: 10),
                    Row(children: [
                      ...List.generate(5, (i) => Icon(
                        i < order.feedback!.overallRating ? Icons.star_rounded : Icons.star_outline_rounded,
                        color: const Color(AppColors.coins), size: 22)),
                      const SizedBox(width: 8),
                      Text('${order.feedback!.overallRating}/5', style: const TextStyle(fontWeight: FontWeight.w700)),
                    ]),
                    if (order.feedback!.comment != null) ...[
                      const SizedBox(height: 6),
                      Text(order.feedback!.comment!, style: TextStyle(fontSize: 13, color: Colors.grey.shade700)),
                    ],
                  ]))
                : OutlinedButton.icon(
                    ...
                    style: OutlinedButton.styleFrom(foregroundColor: const Color(AppColors.coins),
                      side: const BorderSide(color: Color(AppColors.coins))),
                  ),
            ],
            ...
            if (order.canCancel) ...[
              ...
              OutlinedButton(
                ...
                style: OutlinedButton.styleFrom(foregroundColor: const Color(AppColors.error),
  ...
  Widget build(BuildContext context) => Row(
    ...
    children: List.generate(5, (i) => GestureDetector(
      ...
      child: Icon(
        ...
        color: const Color(AppColors.coins), size: 32,
      ),
    )),
  );
}
